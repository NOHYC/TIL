...
mygroup <- c(1,3,1,3,1,3,1,3,1,3,1,3,1,3)
ave(x,mygroup,FUN = mean)
# ave(x, mygroup, FUN = mean) #2  2  2  7  7  7  7  7  7  7
# 원래다루던 데이터 , 그룹 정보 , 집계함수 
# 1. 내부적으로 그룹정보별로 데이터를 나눈다
# 2. 각 그룹에 집계함수를 적용한다.
# 3. 반환된 값을 그룹 길이에 맞는 벡터로 쫙 채운다.
# 4. 반환된 벡터들을 원래 벡터 순서에 맞게 다시 풀어서 최종 결과벡터를 준다
